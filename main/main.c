...
static FtpServerContext ftpServerContext;
static FtpClientConnection ftpConnections[APP_HTTP_MAX_CONNECTIONS];
static uint_t ftpServerCheckUserCallback(FtpClientConnection *connection,
		const char_t *user);
static uint_t ftpServerCheckPasswordCallback(FtpClientConnection *connection,
		const char_t *user, const char_t *password);
static uint_t ftpServerGetFilePermCallback(FtpClientConnection *connection,
		const char_t *user, const char_t *path);

void ftpServerInitStart() {
...
//Get default settings
	...
	ftpServerSettings.maxConnections = APP_HTTP_MAX_CONNECTIONS;
	ftpServerSettings.connections = ftpConnections;
	//Root directory
	strcpy(ftpServerSettings.rootDir, "/littlefs/");
...
static uint_t ftpServerCheckPasswordCallback(FtpClientConnection *connection,
	...
	if (!strcmp(user, "admin") && !strcmp(password, "0000")) {
...
void app_main(void)
{
   ...
   {
      //Debug message
      TRACE_ERROR("Failed to initialize fs!\r\n");
   }
   ...
   strcpy(httpServerSettings.rootDirectory, "/littlefs/");
   ...
   const char_t *param)
{
   ...
   {
      strcpy(connection->buffer, "ESP32-DevKitM");
   }

...
{   esp_vfs_littlefs_conf_t conf =
   ...
   {
      if (ret == ESP_FAIL) {
         TRACE_ERROR("-=SPIFSS fsInit=- Failed to mount or format filesystem\r\n");
      } else{
         if (ret == ESP_ERR_NOT_FOUND)
         {
            TRACE_ERROR("-=SPIFSS fsInit=- Failed to find SPIFFS partition\r\n");
         }
         else
         {
            TRACE_ERROR("-=SPIFSS fsInit=- Failed to initialize SPIFFS (%s)\r\n", esp_err_to_name(ret));
         }
      }
   } else{
      TRACE_INFO("-=SPIFSS fsInit=- SPIFFS partition init OK\r\n");
   }
